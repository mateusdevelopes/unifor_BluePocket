//
//  AddTypeRevenueViewController.swift
//  BluePocket
//
...
class AddTypeRevenueViewController: UIViewController {
    
    @IBOutlet weak var nameTextField: UITextField!
    @IBOutlet weak var addTypeRevenueBT: UIButton!
    ...
    func setUpElements(){
        ...
        Utilities.styleFilledButton(addTypeRevenueBT)
    ...
    @IBAction func addTypeButton(_ sender: Any) {
        if self.nameTextField.text!.isEmpty {
            ...
            db.collection("users").document(self.uid).collection("types_revenue") .addDocument(data: docData)
            { (error) in
                if error != nil {
                    //Show error message
